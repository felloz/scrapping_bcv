...
def save_binance_rate(price):
    try:
        ...
        if last_record.last_update is None:
            store_binance_data(price, last_record.price, last_record.last_update, currency_type)
...
def store_binance_data(price, price_db, last_update_date_db, currency_type):
    try:
        Monitor.create(
            currency='VES_USDT',
